...
feature_gen = config.TgFG(n_bits=config.Less_nbits)
features = feature_gen.fit_transform(df['S'].tolist())
df['FP'] = features

# Saving the parameters
feature_gen.save('Features_Tg1.pkl')
...
Tg_norm, min_value, max_value = normalized

# ANN Estimator
X = torch.from_numpy(np.array(features)).float()
y = torch.from_numpy(Tg_norm).float().reshape(-1, 1)

# Use the ModelTrainer for Tg
trainer = config.ModelTrainer(X=X, y=y, model_class=config.ANN_Tg, k_folds=10,
                              batch_size=20, epochs=300, learning_rate=0.00014,
                              patience=200, min_delta=1e-5, filename='Trained_Tg1.pt', verbose=True)
...
true_Tg = config.reverse(true_values, min_value, max_value)
pred_Tg = config.reverse(pred_values, min_value, max_value)
train_metrics = config.calc_metrics(true_Tg[all_is_train], pred_Tg[all_is_train])
valid_metrics = config.calc_metrics(true_Tg[~all_is_train], pred_Tg[~all_is_train])
...
true_Tg_plot = true_Tg[plot_indices]
pred_Tg_plot = pred_Tg[plot_indices]

# Display detailed metrics
config.disp_metrics(train_metrics, valid_metrics, "ANN model Tg")
...
plotter.plot(true_Tg_plot, pred_Tg_plot, plot_is_train,
             ...
             unit='(°C)',
             title=r'Parity Plot for Glass Transition Temperature (T$_g$) with ANN',
             filename='Parity_Tg1.png',
